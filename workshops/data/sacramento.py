# https://github.com/KMarkert/sacsma/tree/main
# adapted from https://github.com/tanerumit/sacsmaR
import numpy as np

# Use numba for JIT compilation if available, otherwise fall back to plain numpy.
try:
    from numba import njit
except ImportError:
    def njit(func):
        """No-op decorator: returns the function unchanged when numba is not available."""
        return func


@njit
def sacsma(prcp, pet, par):
    ...
    riva   =  0.0       # Riparian vegetation area (decimal fraction)
    side   =  par[13]   # The ratio of deep recharge to channel base flow [-]
    rserv  =  0.3       # Fraction of lower zone free water not transferrable (decimal fraction)
    initstate = [0,0,500,500,500,0]
...
#    prcp = forcings[:, 0]
    ...
    for i in range(n):
        ...
        dlzp  = 1 - (1 - lzpk)**dinc
        dlzs  = 1 - (1 - lzsk)**dinc
        ninc = int(ninc)                      # convert ninc to int
