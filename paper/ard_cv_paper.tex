...
Automatic Relevance Determination (ARD) \citep{tipping2003,tipping2001,mackay1992} is the recommended feature selection method in ShapleyX. ARD operates within a sparse Bayesian learning framework: each weight $w_i$ is assigned an independent Gaussian prior $\mathcal{N}(0, \alpha_i^{-1})$ with precision $\alpha_i$ estimated from the data via marginal likelihood maximisation. As iterations proceed, many $\alpha_i$ are driven to infinity, effectively pruning the corresponding features.

A critical question arises: \textbf{which iteration along the ARD path gives the best predictive model?} ARD's internal convergence criteria---no features added or deleted, and small changes in existing feature precisions---guarantee a stationary point of the marginal likelihood but provide no guarantee of optimal generalisation. An early model may be too sparse (high bias), while the convergence point may overfit (high variance).

We address this through retrospective $K$-fold cross-validation (CV) evaluated along the ARD iteration path. After ARD runs to convergence on the full dataset, each visited model state $\mathcal{M}_t$ is scored via CV, and the state with the highest mean score is retrospectively selected. The key contribution is the \textbf{Bayesian predictive log-likelihood} scoring function: instead of evaluating point-prediction accuracy alone, we score each model by how well its full predictive distribution explains held-out validation data. This is the natural scoring rule for a probabilistic model.
...
The methodology is production-ready: the \texttt{RegressionARD} class in ShapleyX accepts \texttt{cv=True} and \texttt{cv\_method='bayesian'} to activate the full Bayesian CV pipeline with a single parameter change.
